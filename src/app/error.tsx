...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center h-[70vh] px-6 text-center">
      ...
      <h1 className="font-display text-3xl text-ink mb-2 text-balance">Ceva n-a mers bine</h1>
      ...
      {error.digest && <p className="text-[11px] text-ink-faint mt-4">Cod eroare: {error.digest}</p>}
    </div>
  );
}
