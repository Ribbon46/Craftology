...
## [2026-06-04] - Phase 2 · Stages 4–5: Stripe Connect onboarding + 15% checkout
- **Connect onboarding** (`src/actions/connect.ts`): `createSellerOnboardingLink` creates a Stripe **Express (business)** account for an approved seller and returns a hosted Account Link; `syncSellerStripeStatus` re-checks `charges_enabled`/`details_submitted` on return and persists `stripe_onboarded`. The seller page gained a "Configurează plățile (Stripe)" button (approved → onboard → "Plăți configurate ✓").
- **Checkout** (`createCheckoutSession`): platform-owned listings (Deco Kubik) check out to the platform account (100%); marketplace-seller listings use a **direct charge on the seller's connected account** (`{ stripeAccount }`) with `application_fee_amount` = **15%** to the platform — the seller is merchant of record + bears the Stripe fee, per the spec. Gated on the seller being approved + onboarded. New `src/lib/supabase/admin.ts` service-role helper reads the protected Stripe columns.
- Verified the Connect calls against the test key (`scripts/verify-connect.mjs`, context7-checked): account-create / account-link / direct-charge Checkout shapes are all well-formed; build green.
- **One-time steps for you:** (1) complete the Connect **platform profile** at `dashboard.stripe.com/settings/connect/platform-profile` (review loss-management responsibilities) — Stripe blocks creating connected accounts until then; (2) on the Stripe **webhook** endpoint, enable **"events on connected accounts"** so a marketplace sale's `checkout.session.completed` (fired on the seller's account) reaches the sold-marking webhook.

## [2026-06-04] - Phase 2 · Stages 6–7: listing gating + buyer contact + seller dashboard
- **Listing eligibility + cap:** `createListing` now requires the platform owner OR an **approved + Stripe-onboarded** seller; new sellers are capped at **20 active listings until their first sale**. The Sell page gates via `canSell`, routing non-eligible users to `/seller/apply` with the right message (not-a-seller / pending / rejected / suspended / payments-not-set-up).
- **Buyer-facing seller contact** (mom's requirement): listing detail pages show the seller's company + direct contact (email/phone/other) for approved sellers (`fetchSellerPublicById`, RLS-public columns).
- **Seller dashboard** (`/seller/dashboard`): products (active/sold counts + list), "+ Adaugă" shortcut, and a **"Gestionează plățile"** button → single-use Stripe Express dashboard login link (`createStripeLoginLink`). Entry points in Profil → Setări + the approved seller card.
- Shared `src/lib/owner.ts` (platform-owner ids; checkout/listings/seller all use it). E2E: dashboard gate added — **28 passed / 2 skipped**; build green.
- **Phase 2 buildable scope (Stages 1–7) complete.** Remaining: Stage 8 (update Terms/Privacy/Returns for 15% + persoană-juridică + seller liability → lawyer review), the two Stripe dashboard steps above, and end-to-end live testing once the platform profile is done.

## [2026-06-04] - Phase 2 · Stage 8: legal drafts (for lawyer review)
- Drafted the marketplace legal set in Romanian (`docs/legal/`): **Termeni și Condiții**, **Acordul Vânzătorului** (platform↔seller contract — 15% commission, persoană-juridică requirement, seller liability for products/returns, Stripe payments, suspension), **Politica de Confidențialitate** (GDPR — processors, seller data, rights), **Politica de Retururi** (OUG 34/2014 14-day withdrawal + the handmade/personalized exception, OUG 140/2021 conformity guarantee), **Politica de Cookie-uri**, and a README with the `[ ]` company-detail placeholders + applicable-law checklist. All marked **CIORNĂ** — needs lawyer review.
- `scripts/make-legal-pdfs.mjs` renders each draft to its own PDF into `Downloads/Craftology-Legal/` (PDFs only). The in-app `/terms` `/privacy` `/returns` pages will be updated **only after** the lawyer signs off (don't publish unreviewed legal text).
- **Phase 2 feature-complete (Stages 1–8).** Go-live pending: the two Stripe dashboard steps, lawyer sign-off, and filling the company `[ ]` details.
