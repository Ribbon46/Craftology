...
- **Orders & cancellation:** `/seller/dashboard` has tabs Panou / Inventar / **Comenzi** (`?tab=comenzi` deep-links; the new-order email points there). Comenzi = `SellerOrders` (buyer contact, delivery, invoicing, payment split). A seller can refuse an order **at any time** (no admin approval) with a reason from `SELLER_CANCEL_REASONS` → automatic full Stripe refund to the buyer. **Commission rule (owner, sep 2026):** the 10% fee goes back to the seller (`refund_application_fee`) only if the order is cancelled within `COMMISSION_REFUND_WINDOW_HOURS` (48h, `config/app.ts` → `commissionRefundable()`), whoever cancels; after that Craft'zaar keeps it. The buyer always gets 100%. Buyer withdrawal (OUG 34/2014, 14 days) stays unrestricted. All cancel paths share `refundOrder` (restocks via `restore_listing_stock` only for the row it flips; the `charge.refunded` webhook likewise restocks only rows it flips) and `notifyCancellation` (emails the buyer always, the seller when someone else cancelled).
- **Follows:** `FollowButton` on the listing seller block + atelier page; profile tab **Urmăriți** lists them (`getMyFollows`), also reachable from the heart button next to the cart in `SiteHeader` (`/profile?tab=urmariti` — the profile's open tab lives in `?tab=`).
