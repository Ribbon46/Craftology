...
- **Orders & cancellation:** `/seller/dashboard` has tabs Panou / Inventar / **Comenzi** (`?tab=comenzi` deep-links; the new-order email points there). Comenzi = `SellerOrders` (buyer contact, delivery, invoicing, payment split). A seller may refuse an order only within `SELLER_CANCEL_WINDOW_HOURS` (24h, `config/app.ts`) with a reason from `SELLER_CANCEL_REASONS` → automatic full Stripe refund; later cancellations go via `/admin/orders` (no limit). Buyer withdrawal (OUG 34/2014, 14 days) stays unrestricted. All cancel paths share `refundOrder` (restocks via `restore_listing_stock` only for the row it flips; the `charge.refunded` webhook likewise restocks only rows it flips) and `notifyCancellation` (emails the buyer always, the seller when someone else cancelled).
- **Follows:** `FollowButton` on the listing seller block + atelier page; profile tab **Urmăriți** lists them (`getMyFollows`).
